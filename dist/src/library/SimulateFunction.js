 ...
 */
export class VelocityVerletSim {
    /**
    ...
    simulate(deltaT, currState) {
        ...
        let updatedBodies = currState.bodies.map((v) => {
            ...
            updatedBody.position = this.calcNewPos(updatedBody.position, updatedBody.velocity, updatedBody.acceleration, deltaT);
            return updatedBody;
        });
        // a(t + dt) = accel at x(t + dt)
        let newForces = this.forceCalculator.getForces(updatedBodies);
        return new State(updatedBodies.map((b, i) => {
     ...
     * @hidden
     */
    calcNewPos(currPos, currVel, currAccel, deltaT) {
        return currPos
            .clone()
            .add(currVel.clone()
            .multiplyScalar(deltaT))
 ...
 * @category SimulateFunctions
 */
export class SemiImplicitEulerSim {
    /**
    ...
    simulate(deltaT, currState) {
        const updatedBodies = currState.bodies.map((b) => {
            ...
            this.rateUpdate(b.position, updatedVel, deltaT), updatedVel);
        });
        const updatedForces = this.force.getForces(updatedBodies);
        updatedBodies.forEach((b, i) => {
            // a(t + dt) = accel at x(t + dt)
...
export class RungeKutta4Sim {
     ...
     */
    simulate(deltaT, currState) {
        let bodiesK = currState.bodies.map((v) => ({
            // k1v = a(x(t))
        ...
        const updatedBodies = currState.bodies.map((b, i) => {
            ...
            return b.clone(weightedXRate.multiplyScalar(deltaT / 6)
                .add(b.position), weightedVRate.multiplyScalar(deltaT / 6)
                .add(b.velocity));
        });
        const updatedForces = this.force.getForces(updatedBodies);
     ...
     * @hidden
     */
    getInterKV(bodies, bodiesK, krInd, deltaT) {
     ...
     * @hidden
     */
    getInterKX(bodies, bodiesK, kInd, deltaT) {
        // v(t) + k(kInd)v * deltaT
